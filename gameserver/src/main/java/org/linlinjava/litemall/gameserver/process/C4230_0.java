...
public class C4230_0 implements org.linlinjava.litemall.gameserver.GameHandler {
    public void process(ChannelHandlerContext ctx, ByteBuf buff) {
        ...
        for (int i = 0; i < chara.pets.size(); i++) {
            if (((Petbeibao) chara.pets.get(i)).id == id) {
                if (((PetShuXing) ((Petbeibao) chara.pets.get(i)).petShuXing.get(0)).skill >= 50) {
                    owner_id = 10;
                }
                if (((PetShuXing) ((Petbeibao) chara.pets.get(i)).petShuXing.get(0)).penetrate == 1) {
                    owner_id = 0;
                }
