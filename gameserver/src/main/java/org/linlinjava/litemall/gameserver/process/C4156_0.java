...
public class C4156_0 implements org.linlinjava.litemall.gameserver.GameHandler {
    public void process(io.netty.channel.ChannelHandlerContext ctx, io.netty.buffer.ByteBuf buff) {
        ...
        GameObjectChar session1 = GameObjectCharMng.getGameObjectChar(chara1.id);

        if ("request_team_leader".equals(ask_type)) {
            List<Vo_61545_0> vo_61545_0List = GameUtil.a61545(chara1);
            GameObjectChar.send(new org.linlinjava.litemall.gameserver.data.write.M61545_0(), vo_61545_0List);
            org.linlinjava.litemall.gameserver.data.vo.Vo_24505_0 vo_24505_0 = GameUtil.a24505(chara1);
            GameObjectChar.send(new org.linlinjava.litemall.gameserver.data.write.M24505_0(), vo_24505_0);
            ...
            vo_45240_0.para_str = "{}";
            GameObjectCharMng.getGameObjectChar(chara1.id).sendOne(new org.linlinjava.litemall.gameserver.data.write.M45240_0(), vo_45240_0);
        ...
        if (ask_type.equals("request_join")) {
            if (GameObjectCharMng.getGameObjectChar(id).gameTeam != null) {
                ...
                for (int i = 0; i < GameObjectCharMng.getGameObjectChar(id).gameTeam.liebiao.size(); i++) {
                    for (int j = 0; j < ((List) GameObjectCharMng.getGameObjectChar(id).gameTeam.liebiao.get(i)).size(); j++) {
                        if (((Chara) ((List) GameObjectCharMng.getGameObjectChar(id).gameTeam.liebiao.get(i)).get(j)).id == chara.id) {
                ...
                if (!has.booleanValue()) {
                    List<Chara> list = new java.util.ArrayList();
                    list.add(chara);
        ...
        if ("invite_join".equals(ask_type)) {
            ...
            if (GameObjectChar.getGameObjectChar().gameTeam == null) {
                ...
                vo_20480_0.time = 1562593376;
                GameObjectChar.send(new M20480_0(), vo_20480_0);
            } else if (GameObjectChar.getGameObjectChar().gameTeam.duiwu.size() >= 5) {
                return;
            }
