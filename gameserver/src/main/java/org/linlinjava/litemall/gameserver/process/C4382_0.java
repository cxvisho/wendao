...
public class C4382_0 implements org.linlinjava.litemall.gameserver.GameHandler {
    public void process(ChannelHandlerContext ctx, ByteBuf buff) {
        int pet_id = org.linlinjava.litemall.gameserver.data.GameReadTool.readInt(buff);
        ...
        for (int i = 0; i < chara.pets.size(); i++) {
            if (((Petbeibao) chara.pets.get(i)).id == pet_id) {
                for (int j = 0; j < ((Petbeibao) chara.pets.get(i)).petShuXing.size(); j++) {
                    if (((PetShuXing) ((Petbeibao) chara.pets.get(i)).petShuXing.get(j)).no == 23) {
                        chara.yidongsudu = ((PetShuXing) ((Petbeibao) chara.pets.get(i)).petShuXing.get(0)).capacity_level;
                    }
                }
                chara.zuoqiwaiguan = (((PetShuXing) ((Petbeibao) chara.pets.get(i)).petShuXing.get(0)).type + 1000);
                chara.zuowaiguan = typeMounts(((PetShuXing) ((Petbeibao) chara.pets.get(i)).petShuXing.get(0)).type + 1000, chara.menpai, chara.sex - 1);
                chara.zuoqiId = ((Petbeibao) chara.pets.get(i)).id;
            }
        }
        ...
        vo_8425_0.id = pet_id;
        GameObjectChar.getGameObjectChar().gameMap.send(new org.linlinjava.litemall.gameserver.data.write.M8425_0(), vo_8425_0);
